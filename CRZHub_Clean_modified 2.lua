...
if not WindUILoadOk or not WindUILoadResult then
    warn("[العمدة RTA] WindUI failed to load: " .. tostring(WindUILoadResult))
    return
end
...
local DEFAULT_THEME = "العمدة RTA Dark Green"
...
local DISCORD_LINK = "https://discord.gg/rta"
...
local function showDiscordPopup()
    ...
    title.BackgroundTransparency = 1
    title.Text = "العمدة RTA | حقوق RTA\nhttps://discord.gg/rta"
...
local function clearAll()
    ...
    espPlayerAttach = nil
    espEggAttach = nil
    pcall(function() if espBestLabel then espBestLabel:Destroy() end end)
    espBestLabel = nil
end
...
local function updateBestEggLabel()
    if not espEnabled or not bestEggUid then
        pcall(function() if espBestLabel then espBestLabel:Destroy() end end)
        espBestLabel = nil
        return
    end
    ...
    local basePart = data.basePart or getModelBasePart(data.model)
    if not basePart then return end
    data.basePart = basePart

    if espBestLabel and espBestLabel.Parent ~= basePart then
        pcall(function() espBestLabel:Destroy() end)
        espBestLabel = nil
    end

    if not espBestLabel then
        espBestLabel = Instance.new("BillboardGui")
        espBestLabel.Name = "RTA_StrongEggESP"
        espBestLabel.Size = UDim2.fromOffset(190, 58)
        espBestLabel.StudsOffset = Vector3.new(0, 4, 0)
        espBestLabel.AlwaysOnTop = true
        espBestLabel.MaxDistance = 500
        espBestLabel.LightInfluence = 0
        espBestLabel.Parent = basePart
        ...
        background.Parent = espBestLabel
    ...
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    local distance = hrp and math.floor((hrp.Position - basePart.Position).Magnitude) or 0
    local rarity = getEggRarityName(data.record)
    local info = espBestLabel:FindFirstChild("Background")
    local text = info and info:FindFirstChild("Info")
    if text then
        text.Text = "أقوى بيضة\n" .. rarity .. "  |  " .. tostring(distance) .. "m"
    end
end
...
local EggEspToggle = EggTab:Toggle({
    Title = "ESP البيضة القوية",
    Desc = "يحدد أقوى بيضة ويعرض اسمها وندرتها ومسافتها.",
    Icon = "eye",
    Type = "Toggle",
    Value = espEnabled,
...
SettingsTab:Keybind({
    Title = "UI Keybind",
    Desc = "مفتاح فتح/إغلاق واجهة العمدة RTA.",
    Icon = "keyboard",
    Value = "K",
    CanChange = true,
...
SettingsTab:Button({
    Title = "Join Discord",
    Desc = "نسخ رابط سيرفر العمدة RTA.",
    Icon = "message-circle",
    Callback = function()
...
if type(hookmetamethod) == "function" and type(getnamecallmethod) == "function" then
    ...
    warn("[العمدة RTA] Namecall hook unavailable; Anti Rejoin/Kick hook is disabled.")
end
